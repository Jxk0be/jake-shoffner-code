...
<div class={`transition-all z-30 text-[125%] uppercase bg-[#0b0b0b] font-semibold min-w-[250px] text-white lg:hidden ease-in-out duration-150 ${menuOpen ? 'right-0' : 'md:right-[-40%] right-[-250px]'} fixed pt-[90px] w-[250px] top-0 md:w-[40%] h-full border-l border-gray-600`}>
    <ul class="text-left font-bold">
        ...
        <a href="about" on:click={() => handleClose()}>
            <li class="p-5 border-b border-gray-600 duration-150 ease-in-out transition-all hover:text-[#38c5fd]">About</li>
        </a>
        <a href="projects" on:click={() => handleClose()}>
            <li class="p-5 border-b border-gray-600 duration-150 ease-in-out transition-all hover:text-[#38c5fd]">Projects</li>
        </a>
        <a href="experience" on:click={() => handleClose()}>
            <li class="p-5 border-b border-gray-600 duration-150 ease-in-out transition-all hover:text-[#38c5fd]">Experience</li>
        </a>
    </ul>
</div>